class WinScreen {
  ...
  _build(data) {
    ...
    title.anchor.set(0.5); title.x = 540; title.y = 180;
    ...
    for (let i = 0; i < (data.top3 || []).length; i++) {
      ...
      bg.beginFill(trophyColors[i], 0.2); bg.drawRoundedRect(250, y - 10, 580, 80, 12); bg.endFill();
      bg.lineStyle(2, trophyColors[i], 0.5); bg.drawRoundedRect(250, y - 10, 580, 80, 12);
      ...
      if (p.avatarUrl) {
        try {
          ...
          avatarSprite.width = 55; avatarSprite.height = 55;
          avatarSprite.x = 295; avatarSprite.y = y + 30;
          const mask = new PIXI.Graphics();
          mask.beginFill(0xffffff); mask.drawCircle(295, y + 30, 26); mask.endFill();
          this.container.addChild(mask);
          avatarSprite.mask = mask;
          this.container.addChild(avatarSprite);
          const ring = new PIXI.Graphics();
          ring.lineStyle(3, trophyColors[i]); ring.drawCircle(295, y + 30, 28);
      ...
      text.anchor.set(0.5); text.x = 570; text.y = y + 30;
    ...
    if (data.king) {
      const kingText = new PIXI.Text(
        ...
        { fontFamily: 'Arial', fontSize: 22, fill: 0xffd700, align: 'center' }
      );
  ...
  _startConfetti() {
    for (let i = 0; i < 100; i++) {
      ...
      const colors = [0xffd700, 0xff4400, 0x00ff88, 0x4488ff, 0xff69b4];
